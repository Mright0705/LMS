    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="作业点评" prop="zuoyedianping">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="作业点评"
                  v-model="ruleForm.zuoyedianping" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.zuoyedianping" label="作业点评" prop="zuoyedianping">
                    <span>{{ruleForm.zuoyedianping}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xueshengxingming = true;
          ...
          if(o=='zuoyedianping'){
            this.ruleForm.zuoyedianping = obj[o];
	    this.ro.zuoyedianping = true;
            continue;
          }
          if(o=='chengji'){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zuoyechengji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zuoyechengjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
